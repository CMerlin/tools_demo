...
var formSet = function() {
	return {
		...
		//为password赋值
		setPasswordValue: function(formID, name, value) {
			if(value != 'undefined') {
				jQuery(formID + '[name="' + name + '"]:password').val(value);
			}
		},
		
		//获取文本框的值
		getPasswordValue: function(formID, name) {
			return jQuery(formID + '[name="' + name + '"]:password').val();
		},
			...
			if(keyvalueObj && typeof keyvalueObj == 'object') {
				...
				if(name && value && type){
					switch(type) {
						...
						case 'password': {
							formSet.setPasswordValue(formID, name, value);
						}
						default: {
