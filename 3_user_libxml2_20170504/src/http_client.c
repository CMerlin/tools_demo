...
/******************************************************************************
 * Description：执行一条shall命令，将一些信息写到配置文件中，读文件，获取http服务器端口号和ip
 * Output server：获取到的目标数据
 * Return：0-成功 -1-失败
 * *****************************************************************************/
sint32 get_network_attr(NETWORK_ATTR_S *server)
{
	sint32 port = 80;
	uint8 ip[64] = {"127.0.0.1"}, path[64] = {"/tmp/ports"};
	uint8 cmd[1024] = {"netstat -anplt|grep LIST|grep \\:\\:\\:|grep sys|cut -d \\: -f 4 > /tmp/ports"};
	//netstat -anplt|grep LIST|grep \:\:\:|grep sys|cut -d \: -f 4 > /tmp/ports
	if(0 > system((char*)cmd))
	{
		trace(ERROR, "[%s]:system=%s cmd=%s LINE:%d\n", __func__, strerror(errno), cmd, __LINE__);
		return -1;
	}
	if(0 > get_config_info(path, &port))
	{
		trace(ERROR, "[%s]:read config file faild! LINE:%d\n", __func__, __LINE__);
		return -1;
	}
	server->port = port;
	strcpy(server->ip, (char*)ip);

	return 0;
}

static NETWORK_ATTR_S connect_info;
static pthread_mutex_t connect_info_lock;
/*******************************************************
 * Description：更新当前的连接状态
 * Input attr：需要更新的信息
 * Return：0-成功； -1-失败
 * *****************************************************/
sint32 isapi_set_connect_status(NETWORK_ATTR_S *attr)
{
	pthread_mutex_lock(&connect_info_lock);
	connect_info = (*attr);
	pthread_mutex_unlock(&connect_info_lock);

	return 0;
}

/*******************************************************
 * Description：获取当前的客服端和服务器端的连接状态信息
 * Output attr：连接信息
 * Return：0-成功； -1-失败
 * *****************************************************/
sint32 isapi_get_connect_status(NETWORK_ATTR_S *attr)
{
	pthread_mutex_lock(&connect_info_lock);
	(*attr) = connect_info;
	pthread_mutex_unlock(&connect_info_lock);
...
//static HTTP_ATTR_S shttp_attr = {"127.0.0.1", ALIVE};
static HTTP_ATTR_S shttp_attr;
static pthread_mutex_t shttp_attr_lock;
/*******************************************************
 * Description：设置http相关的属性信息，这些信息将会用来封装http请求包的头
 * Input attr：创建http请求包所需要的信息
 * Return：0-成功； -1-失败
 * *****************************************************/
sint32 isapi_set_http_attr(HTTP_ATTR_S *attr)
{

	pthread_mutex_lock(&shttp_attr_lock);
	shttp_attr = (*attr);
	pthread_mutex_unlock(&shttp_attr_lock);
	return 0;
}

/*******************************************************
 * Description：获取http属性设置相关的信息
 * Input attr：创建http请求包所需要的信息
 * Return：0-成功; -1-失败
 * *****************************************************/
sint32 isapi_get_http_attr(HTTP_ATTR_S *attr)
{
	pthread_mutex_lock(&shttp_attr_lock);
	(*attr) = shttp_attr;
	pthread_mutex_unlock(&shttp_attr_lock);
